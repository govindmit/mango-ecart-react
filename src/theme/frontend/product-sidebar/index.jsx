...
export default function ProductBanner() {
  ...
  const uniqueMyParentChildArray = myParentChildArray.filter(
    (item, index, self) => {
  ...
  const handleCheckboxClick = (id) => {
    const selectedColorArray = selectedItem
      .split(",")
      .filter((item) => item !== "");
  ...
  const handleSizeCheckboxClick = (id) => {
    const selectedSizeArray = selectedSize
      .split(",")
      .filter((item) => item !== "");
  ...
  useEffect(() => {
    ...
    const filterString = `?page=${page}&pageSize=${pageSize}&filter=true${Object.keys(
      filters
    )
    ...
    <div className="myDivxyz">
                ...
                <ListItemButton>
                  <Checkbox
                    ...
                    checked={selectedSize
                      .split(",")
                      .includes(sizeOption.optionValue)}
                    tabIndex={-1}
                    disableRipple
                    disablePadding
                    onClick={() =>
                      handleSizeCheckboxClick(sizeOption.optionValue)
                    }
                  />
      ...
      <div>
        <div className="icon-container">
          <CalendarViewMonthIcon onClick={() => handleProductShow()} />
          <ListIcon onClick={() => handleProductShow()} />
        </div>
        <div>
          {isProductData ? (
            <ProductDataCard
